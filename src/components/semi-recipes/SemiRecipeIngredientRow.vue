        ...
        <q-item v-bind="scope.itemProps">
          <q-item-section>
            ...
            <q-item-label caption>
              {{ scope.opt.unit }} | цена: {{ formatMoney(scope.opt.costPrice / scope.opt.baseRatio) }}/{{ scope.opt.baseUnit }}
            </q-item-label>
          </q-item-section>
        </q-item>
      </template>
...
export default defineComponent({
  name: 'SemiRecipeIngredientRow',

  props: {
    ingredient: {
      type: Object as PropType<{ ingredientId: number; quantity: number }>,
      required: true
    },
    ingredients: { type: Array as PropType<Ingredient[]>, required: true }
  },
