...
class CustomRestMethods
{
    ...
    const HL_SEMI_RECIPE_INGREDIENTS = 10;  
    ...
    public static function createProduct($data = [])
{
    ...
    /**
     * Получить список категорий (разделов) для ингредиентов
     */
    ...
    /**
     * Создать новую категорию для ингредиентов
     */
    ...
    public static function deleteIngredientCategory($data = [])
    {
        ...
        // Проверяем, есть ли ингредиенты в категории
        $res = \CIBlockElement::GetList(
            [],
    ...
    /**
     * Получить список рецептов полуфабрикатов
     */
    public static function getSemiRecipes($data = [])
    {
        Loader::includeModule('highloadblock');
        Loader::includeModule('iblock');
        ...
        if (!$hlblock) {
            throw new \Exception('HL-блок SemiFinishedRecipes не найден');
        }
        ...
        if (!empty($data['semiFinishedId'])) {
            $filter['=UF_SEMI_FINISHED_ID'] = (int)$data['semiFinishedId'];
        }
        ...
        $result = [];
        while ($row = $res->fetch()) {
            // Получаем название полуфабриката
            $semiName = self::getSemiFinishedName($row['UF_SEMI_FINISHED_ID']);
            
            // Получаем ингредиенты (с названиями)
            $ingredients = self::getSemiRecipeIngredients($row['ID']);
            
            $result[] = [
                'id' => (int)$row['ID'],
                'semiFinishedId' => (int)$row['UF_SEMI_FINISHED_ID'],
                'semiFinishedName' => $semiName,
                'name' => $row['UF_NAME'],
                'outputQuantity' => (float)$row['UF_OUTPUT_QUANTITY'],
                'outputUnit' => $row['UF_OUTPUT_UNIT'],
                'instructions' => $row['UF_INSTRUCTIONS'] ?? '',
    ...
    /**
     * Получить ингредиенты рецепта полуфабриката
     */
    private static function getSemiRecipeIngredients($recipeId)
    {
        Loader::includeModule('highloadblock');
        Loader::includeModule('iblock');
        
        $hlId = self::HL_SEMI_RECIPE_INGREDIENTS; // ID HL-блока SemiRecipeIngredients (создадим ниже)
        ...
        while ($row = $res->fetch()) {
            ...
            // Получаем себестоимость ингредиента (для расчёта)
            $ingredient = self::getIngredientById($row['UF_INGREDIENT_ID']);
            $cost = $ingredient ? ($ingredient['costPrice'] / $ingredient['baseRatio']) * $row['UF_QUANTITY'] : 0;
    ...
    /**
     * Получить название полуфабриката
     */
    private static function getSemiFinishedName($id)
    {
        Loader::includeModule('iblock');
        $res = \CIBlockElement::GetByID($id);
        if ($item = $res->Fetch()) {
    ...
    /**
     * Создать рецепт полуфабриката
     */
    public static function createSemiRecipe($data = [])
    {
        Loader::includeModule('highloadblock');
        
        $required = ['semiFinishedId', 'name', 'outputQuantity', 'outputUnit', 'ingredients'];
        foreach ($required as $field) {
            if (empty($data[$field])) {
                throw new \Exception("Field {$field} is required");
            }
        ...
        $fields = [
            'UF_SEMI_FINISHED_ID' => (int)$data['semiFinishedId'],
            'UF_NAME' => $data['name'],
            'UF_OUTPUT_QUANTITY' => (float)$data['outputQuantity'],
            'UF_OUTPUT_UNIT' => $data['outputUnit'],
            'UF_INSTRUCTIONS' => $data['instructions'] ?? '',
            'UF_CREATED_AT' => new DateTime()
        ];
        ...
        self::saveSemiRecipeIngredients($recipeId, $data['ingredients']);
        
        return ['id' => $recipeId];
    }

    /**
     * Сохранить ингредиенты рецепта полуфабриката
     */
    private static function saveSemiRecipeIngredients($recipeId, $ingredients)
    {
        Loader::includeModule('highloadblock');
        
        $hlId = self::HL_SEMI_RECIPE_INGREDIENTS;
        $hlblock = \Bitrix\Highloadblock\HighloadBlockTable::getById($hlId)->fetch();
        $entity = \Bitrix\Highloadblock\HighloadBlockTable::compileEntity($hlblock);
        $dataClass = $entity->getDataClass();
        ...
        // Добавляем новые
        foreach ($ingredients as $ing) {
            // Получаем базовую единицу ингредиента
            $ingredient = self::getIngredientById($ing['ingredientId']);
            $unit = $ingredient ? $ingredient['baseUnit'] : 'г';
    ...
    /**
     * Обновить рецепт полуфабриката
     */
    public static function updateSemiRecipe($data = [])
    {
        $id = (int)($data['id'] ?? 0);
        if (!$id) {
            throw new \Exception('ID рецепта не указан');
        }
        ...
        $fields = [];
        if (!empty($data['name'])) $fields['UF_NAME'] = $data['name'];
        if (isset($data['outputQuantity'])) $fields['UF_OUTPUT_QUANTITY'] = (float)$data['outputQuantity'];
        if (!empty($data['outputUnit'])) $fields['UF_OUTPUT_UNIT'] = $data['outputUnit'];
        if (isset($data['instructions'])) $fields['UF_INSTRUCTIONS'] = $data['instructions'];
        ...
        if (isset($data['ingredients'])) {
            self::saveSemiRecipeIngredients($id, $data['ingredients']);
        }
    ...
    /**
     * Удалить рецепт полуфабриката
     */
    public static function deleteSemiRecipe($data = [])
    {
        $id = (int)($data['id'] ?? 0);
        if (!$id) {
            throw new \Exception('ID рецепта не указан');
        ...
        // Удаляем ингредиенты
        $ingHlId = self::HL_SEMI_RECIPE_INGREDIENTS;
        $ingHlblock = \Bitrix\Highloadblock\HighloadBlockTable::getById($ingHlId)->fetch();
        $ingEntity = \Bitrix\Highloadblock\HighloadBlockTable::compileEntity($ingHlblock);
    ...
    /**
     * Производство полуфабриката по рецепту
     * @param array $data Параметры:
     *  - semiFinishedId: ID полуфабриката
     *  - sourceQuantity: количество сырья (в единицах измерения сырья) или количество готового продукта
     *  - mode: 'source' (указано количество сырья) или 'target' (указано количество готового продукта)
     */
    public static function produceSemiFinished($data = [])
    {
        ...
        foreach ($ingredients as $ing) {
            ...
            $neededQuantity = $ing['quantity'] * $producedQuantity;
        ...
        // 3. Отходы
        if (!empty($data['waste']) && !empty($data['waste']['productId']) && $data['waste']['quantity'] > 0) {
            self::addMovement([
                'productId' => $data['waste']['productId'],
                'type' => 'income',
                'quantity' => $data['waste']['quantity'],
                'price' => 0,
                'documentType' => 'waste',
                'comment' => "Отходы от производства полуфабриката ID {$semiFinishedId}"
            ]);
        }

        // 4. Потери
        if (!empty($data['loss']) && $data['loss'] > 0) {
            self::addMovement([
                'productId' => $semiFinishedId,
                'type' => 'write-off',
                'quantity' => $data['loss'],
                'price' => 0,
                'documentType' => 'loss',
                'comment' => "Потери при производстве полуфабриката ID {$semiFinishedId}"
            ]);
        }

        // 5. Обновляем себестоимость
        \CIBlockElement::SetPropertyValuesEx(
            $semiFinishedId,
    ...
    private static function saveProductionDetails($productionMovementId, $ingredients)
    {
        ...
        $hlId = self::HL_PRODUCTION_DETAILS; // ID=9
        $hlblock = \Bitrix\Highloadblock\HighloadBlockTable::getById($hlId)->fetch();
        $entity = \Bitrix\Highloadblock\HighloadBlockTable::compileEntity($hlblock);
        $dataClass = $entity->getDataClass();
    ...
    /**
     * Получить детали производства по ID движения
     */
    private static function getProductionDetails($productionMovementId)
    {
        Loader::includeModule('highloadblock');

        $hlId = self::HL_PRODUCTION_DETAILS;
        $hlblock = \Bitrix\Highloadblock\HighloadBlockTable::getById($hlId)->fetch();
        $entity = \Bitrix\Highloadblock\HighloadBlockTable::compileEntity($hlblock);
        $dataClass = $entity->getDataClass();
    ...
    private static function getSemiFinishedById($id)
    {
        Loader::includeModule('iblock');
        $res = \CIBlockElement::GetByID($id);
        if ($fields = $res->GetNext()) {
    ...
    /**
     * Получить историю производства полуфабрикатов
     */
    public static function getProductionHistory($data = [])
    {
        ...
        // Дополним информацией о продукте
        foreach ($movements as &$mov) {
            $product = self::getSemiFinishedById($mov['UF_PRODUCT_ID']);
    ...
    public static function revertProduction($data = [])
    {
        ...
        // Получаем исходное движение
        $movement = StockMovementHelper::getMovementById($movementId);
        if (!$movement) {
            throw new \Exception('Движение не найдено');
        }

        // Проверяем, что это действительно производство
        if ($movement['UF_DOCUMENT_TYPE'] != self::getEnumId('UF_DOCUMENT_TYPE', 'production')) {
            throw new \Exception('Это не производственная операция');
        }

        // Получаем детали производства
        $details = self::getProductionDetails($movementId);
        if (empty($details)) {
            throw new \Exception('Детали производства не найдены. Невозможно отменить.');
        }

        // 1. Списать произведённый полуфабрикат (outcome)
        self::addMovement([
    ...
    public static function createSemiFinished($data = [])
    {
        ...
        // Сохраняем ингредиенты (если есть)
    ...
    private static function saveSemiFinishedIngredients($semiFinishedId, $ingredients)
    {
        ...
        $hlId = self::HL_SEMI_RECIPES; // ID HL-блока SemiFinishedRecipes
        $hlblock = \Bitrix\Highloadblock\HighloadBlockTable::getById($hlId)->fetch();
        $entity = \Bitrix\Highloadblock\HighloadBlockTable::compileEntity($hlblock);
        $dataClass = $entity->getDataClass();
        ...
        foreach ($ingredients as $ing) {
            $dataClass::add([
                ...
                'UF_ITEM_TYPE' => $ing['itemType'] ?? 'ingredient',
                'UF_ITEM_ID' => (int)$ing['ingredientId'],
                'UF_QUANTITY' => (float)$ing['quantity'],
                'UF_UNIT' => $ing['unit'] ?? 'г',
            ]);
        }
    ...
    public static function updateProduction($data = [])
    {
        ...
        // Проверяем, что это производство
        if ($movement['UF_DOCUMENT_TYPE'] != self::getEnumId('UF_DOCUMENT_TYPE', 'production')) {
            throw new \Exception('Это не производственная операция');
        }
        ...
        // Отменяем старую операцию (сторно)
        self::revertProduction(['movementId' => $movementId]);

        // Создаём новую с обновлённым количеством
        $semiFinishedId = $movement['UF_PRODUCT_ID'];

        // Подготавливаем новые ингредиенты (масштабируем)
        $scaledIngredients = [];
        foreach ($details as $detail) {
            $scaledIngredients[] = [
                'productId' => $detail['UF_INGREDIENT_ID'],
                'quantity' => $detail['UF_QUANTITY'] * $ratio,
                'price' => $detail['UF_PRICE'],
                'unit' => $detail['UF_UNIT']
            ];
        }

        // Рассчитываем новую себестоимость
        $totalCost = 0;
